...
const Cart = () => {
  ...
  const handleCheckout = async () => {
    try {
      ...
      // ✅ backend order creation (no localStorage)
      await createOrder({
        customerId,
        productIds,
        quantities,
        // optional extra fields (we'll add these in backend below)
        ...
        <div>
          <Card className="border-border/50 sticky top-24">
            ...
            <CardFooter className="flex-col gap-3">
              <Button className="w-full btn-gradient" size="lg" onClick={handleCheckout} disabled={isProcessing}>
                {isProcessing ? (
                  "Processing..."
                ) : (
                  <>
              ...
              <p className="text-xs text-muted-foreground text-center">🔒 Demo checkout - no real payment processed</p>
            </CardFooter>
          </Card>
        </div>
      </div>
