...
import { Plus, Search, Edit, Trash2, X, PlusCircle, Image as ImageIcon } from "lucide-react";
...
const Products = () => {
  ...
  const [formData, setFormData] = useState<Product>({
    id: "", name: "", category: "", price: "", material: "", dimensions: "", capacity: "",
    variants: [{ color: "", frontView: null, backView: null, insideView: null }]
  });
  ...
  const handleVariantTextChange = (index: number, field: keyof ProductVariant, value: string) => {
    const newVariants = [...formData.variants];
    newVariants[index] = { ...newVariants[index], [field]: value };
    setFormData({ ...formData, variants: newVariants });
  };

  const handleVariantFileChange = (index: number, field: keyof ProductVariant, e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  const handleAddVariant = () => {
    setFormData({
      ...formData,
      variants: [...formData.variants, { color: "", frontView: null, backView: null, insideView: null }]
    });
  };
  ...
  const handleOpenForm = (product?: Product) => {
    if (product) {
      ...
      setFormData({
        id: `PRD-00${products.length + 1}`, name: "", category: "", price: "", material: "", dimensions: "", capacity: "",
        variants: [{ color: "", frontView: null, backView: null, insideView: null }]
      });
      setEditingId(null);
    }
  ...
  const uploadImageToServer = async (file: File): Promise<string | null> => {
    ...
    try {
      const response = await fetch("http:// :8082/api/upload/image", {
        method: "POST",
        body: imgData,
      });
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    for (let i = 0; i < formData.variants.length; i++) {
      ...
      if (variant.frontView instanceof File) {
        payload.variants[i].frontView = await uploadImageToServer(variant.frontView);
      }
      if (variant.backView instanceof File) {
        payload.variants[i].backView = await uploadImageToServer(variant.backView);
      }
      if (variant.insideView instanceof File) {
        payload.variants[i].insideView = await uploadImageToServer(variant.insideView);
      }
    }
    ...
    try {
      const url = editingId 
        ? `http://localhost:8082/api/products/${editingId}` 
  ...
  return (
    <DashboardLayout title="Products" subtitle="Manage your bag products and catalog">
            ...
            {filteredProducts.map((product) => (
              <tr key={product.id} className="border-b border-border hover:bg-muted/20 transition-colors">
                ...
                <td className="px-6 py-4">
                  ...
                  <div className="text-xs text-muted-foreground">{product.capacity}</div>
                </td>
                <td className="px-6 py-4 text-xs text-muted-foreground">
                  {product.variants.map(v => v.color).filter(Boolean).join(", ") || "No colors added"}
                </td>
            ...
            <form onSubmit={handleSave} className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <Input name="material" value={formData.material} onChange={handleInputChange} required />
                </div>
                
                <div className="space-y-2">
                  <label className="text-sm font-medium">Dimensions</label>
                  <Input name="dimensions" placeholder="e.g. 15 x 10 x 5 in" value={formData.dimensions} onChange={handleInputChange} required />
                </div>
                <div className="space-y-2">
                  <label className="text-sm font-medium">Capacity</label>
                  <Input name="capacity" placeholder="e.g. 20L" value={formData.capacity} onChange={handleInputChange} required />
                </div>

                <div className="md:col-span-2 pt-4 border-t border-border space-y-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="text-sm font-semibold">Color Variants & Media</h3>
                      <p className="text-xs text-muted-foreground">Upload specific images for each color variation.</p>
                    </div>
                  ...
                  <div className="space-y-4">
                    {formData.variants.map((variant, index) => (
                      <div key={index} className="p-4 bg-muted/30 border border-border rounded-lg relative grid grid-cols-1 md:grid-cols-3 gap-4">
                        ...
                        <div className="space-y-2 md:col-span-3 pr-10">
                          ...
                          <Input 
                            ...
                            value={variant.color} 
                            onChange={(e) => handleVariantTextChange(index, "color", e.target.value)} 
                            required 
                          />
                        </div>
                        ...
                        <div className="space-y-2">
                          ...
                          <Input 
                            type="file" 
                            accept="image/*"
                            onChange={(e) => handleVariantFileChange(index, "frontView", e)} 
                          ...
                          />
                          <p className="text-[10px] text-muted-foreground truncate" title={getFileDisplay(variant.frontView)}>
                        ...
                        <div className="space-y-2">
                          ...
                          <Input 
                            type="file" 
                            accept="image/*"
                            onChange={(e) => handleVariantFileChange(index, "backView", e)} 
                            className="cursor-pointer file:text-primary file:bg-primary/10 file:border-0 file:rounded-md file:mr-2 file:px-2 file:py-1 file:text-xs"
                          />
                          <p className="text-[10px] text-muted-foreground truncate" title={getFileDisplay(variant.backView)}>
                        ...
                        <div className="space-y-2">
                          ...
                          <Input 
                            type="file" 
                            accept="image/*"
                            onChange={(e) => handleVariantFileChange(index, "insideView", e)} 
                            className="cursor-pointer file:text-primary file:bg-primary/10 file:border-0 file:rounded-md file:mr-2 file:px-2 file:py-1 file:text-xs"
                          />
                          <p className="text-[10px] text-muted-foreground truncate" title={getFileDisplay(variant.insideView)}>
